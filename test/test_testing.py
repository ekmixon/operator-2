...
class TestHarness(unittest.TestCase):
    ...
    def test_add_storage_before_harness_begin(self):
        ...
        self.assertSetEqual(
            {self._extract_storage_index(stor_id) for stor_id in stor_ids},
...
class RelationEventCharm(RecordingCharm):
    ...
    def _observe_relation_event(self, event_name, event):
        unit_name = event.unit.name if event.unit is not None else None
        app_name = event.app.name if event.app is not None else None
...
class ContainerEventCharm(RecordingCharm):
    ...
    def _observe_container_event(self, event_name, event: PebbleReadyEvent):
        container_name = event.workload.name if event.workload is not None else None
        self.changes.append(
...
class _PebbleStorageAPIsTestMixin:
    ...
    def test_make_dir_with_permission_mask(self):
        ...
        self.assertEqual(
            [f for f in files if f.path == f'{self.prefix}/dir1'][0].permissions,
            0o700,
        )

        self.assertEqual(
            [f for f in files if f.path == f'{self.prefix}/dir2'][0].permissions,
            0o777,
        )
...
class TestPebbleStorageAPIsUsingMocks(
    ...
    def test_make_dir_with_ownership(self):
        client = self.client
        client.make_dir(
            f'{self.prefix}/dir1', user_id=1, user="foo", group_id=3, group="bar"
        )
